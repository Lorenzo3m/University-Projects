...
def decode_digits(digits: List[int], bases: List[int]) -> int:
    ...
    for index in range(len(digits)):
        result_int = result_int + digits[index]*bases[index]**index
...
def generate_digits(bases : List[int] ) -> List[List[int]]:
    ...
    '''
    # WRITE YOUR CODE HERE
    size_of_list = len(bases)
    ratio_of_numbers = 1
    final_list_size = 1
    all_possible_combinations = []
    
    for index in range(size_of_list):
        ratio_of_numbers = ratio_of_numbers*bases[index]
        final_list_size = final_list_size*bases[index]
    
    for index in range(final_list_size):
        all_possible_combinations.append([])
        
    for index in range(size_of_list):
        ratio_of_numbers = int(ratio_of_numbers/bases[index])
        counter_list = 0
        counter_ratio = 0
        while counter_ratio < int(final_list_size/ratio_of_numbers):
            for index2 in range(bases[index]):
                for r in range(ratio_of_numbers):
                    all_possible_combinations[counter_list].append(index2)
                    counter_list += 1
                counter_ratio += 1
...
def find_doubles(bases : List[int]) -> Set[int]:
    ...
    all_combinations = generate_digits(bases)
    test = set()
    doubles = set()
    
    for index in range(len(all_combinations)):
        calc = 0
        for index2 in range(len(all_combinations[index])):
            base = int(bases[index2])
            num = int(all_combinations[index][index2])
            calc = calc + num*base**index2
