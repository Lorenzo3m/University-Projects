...
def jigsaw(puzzle_image: str, plain_image: str, tile_size:int, encrypted_file: str, plain_file: str) -> list[str]:
    #get images, get keys, store keys in list, translate file, update to file plain and then return the list
    p_image = images.load(puzzle_image)
    image = images.load(plain_image)
    keys = []
    row_tiles = len(image)//tile_size
    col_tiles = len(image[0])//tile_size
    
    for i in range(row_tiles):
        y = i + 1
        key = ''
        for i2 in range(col_tiles):
            m = i2 + 1
            flipped = []
            ninety = []
            twoseven = []
            ...
            for i3 in range(i*tile_size, y*tile_size):
                semi_ninety = []
                for i4 in range(i2*tile_size, m*tile_size):
                    
                    semi_ninety.append(p_image[i4 - i2*tile_size + i*tile_size][i3 + i2*tile_size - i*tile_size])
                    
                semi_ninety.reverse()
                ninety.append(semi_ninety) 
                flipped.append([p_image[y*tile_size - i3 + i*tile_size - 1][m*tile_size - i4 + i2*tile_size - 1] for i4 in range(i2*tile_size, m*tile_size)])
                twoseven.append([p_image[i4 - i2*tile_size + i*tile_size][i3 + i2*tile_size - i*tile_size] for i4 in range(i2*tile_size, m*tile_size)])
                original.append([image[i3][i4] for i4 in range(i2*tile_size, m*tile_size)])
            twoseven.reverse()
            
            key += key_finder(original, ninety, twoseven, flipped)

        keys.append(key)
    
    return keys

def key_finder(o, n, two, f):
